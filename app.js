...
const DB = (() => {
  // Firebase path helpers — convert key like "ml:log:2026-03-22" to "ml/log/2026-03-22"
  const toPath = k => k.replace(/:/g, "/");
...
function Morning({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    ch: /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Lbl, {
    ...
    }, "Glasses today:"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 5,
        alignItems: "center"
      ...
      style: {
        ...
        background: glasses >= n && n > 0 ? "rgba(96,165,250,.15)" : "transparent",
        cursor: "pointer",
...
function App() {
  ...
  const loadAll = async () => {
    ...
    if (pan && pan.length > 0) {
      ...
      // First launch — fetch seed from separate file to keep JS bundle lean
      try {
        const res = await fetch('/pantry-seed.json');
        if (res.ok) {
          const seed = await res.json();
          setPantryItems(seed);
          await DB.set(KEYS.pantry(), seed);
          console.log("[Mission Log] Pantry seeded with", seed.length, "items");
        }
      } catch (e) {
        console.log("[Mission Log] No pantry seed loaded:", e.message);
      }
    }
    setSetupDone(!!sd);
